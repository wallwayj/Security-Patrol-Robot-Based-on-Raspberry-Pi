...
function setOption(chart, xdata, ydata) {
  const option = {
    ...
    yAxis: {
      ...
      axisLabel: {
        color: '#666666',
        formatter: '{value}ppm' // 设置y轴刻度值的格式为温度单位（℃）
      }
    },
    series: [{
      name: 'TDS',
...
Page({
  ...
  init_echart: function () {
    this.Component = this.selectComponent('#mychart-tds');
  ...
  getOption: function () {
    ...
    wx.request({
      ...
      success: function (e) {
        console.log("获取成功", e);
        var ra = e.data.data.datastreams[10].datapoints[0].value;
        that.setData({
          // random:e.data.data.datastreams[1].datapoints[0].value,
        });
        console.log("randomyy==", e.data.data.datastreams[2].datapoints[0].value);
