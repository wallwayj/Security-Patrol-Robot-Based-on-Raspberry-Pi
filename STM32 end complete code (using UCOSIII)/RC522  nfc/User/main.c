...
void Read_Card(void);
#define kup GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)
#define k0 GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_4)
#define k1 GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_3)
#define k2 GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_2)
#define sup 1
#define s0 2
...
void key_init(void)
{
  ...
  ky.GPIO_Pin=GPIO_Pin_0;
	ky.GPIO_Mode=GPIO_Mode_IPD;
	GPIO_Init(GPIOA, &ky);
	
	ky.GPIO_Pin=GPIO_Pin_4|GPIO_Pin_2|GPIO_Pin_3;
	ky.GPIO_Mode=GPIO_Mode_IPU;
	GPIO_Init(GPIOE, &ky);
...
u8 ks(u8 flag)
{
	...
	if(kk&&(k0==0||k1==0||k2==0||kup==1))
	{
		Delay_ms(10);
		kk=0;
		if(kup==1)return sup;
		else if(k0==0)return s0;
		else if(k1==0)return s1;
		else if(k2==0)return s2;
	}
	else if(kup==0&&k1==1&&k2==1&&k0==1)kk=1;
	return 0;
}
...
u8 l=0;	Key_Init();
	...
	key_init();OLED_Init();OLED_ShowString(1,1,"  Welcome Home! ");
	while(1)
	{	
//		KeyNum = Key_Scan();
		l=ks(0);
		if(l==sup)KeyNum=1;		
		else if(l==s1)KeyNum=2;
		else if(l==s2)KeyNum=3;
