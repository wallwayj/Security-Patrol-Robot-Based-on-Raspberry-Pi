...
char *str[4] = {"POST /devices/1202725242/datapoints HTTP/1.1",
				"api-key:YJ=3hqMFW4oW1=ibQWOgR0NSEGY=",//YJ=3hqMFW4oW1=ibQWOgR0NSEGY=
...
u16 esp8266_get_data(char *vStr)
{
	...
	u16 value = 0;
	char *back;
	char temp[160] = "GET /devices/1202725242/datastreams/";
...
void esp8266_start_trans(void)
{
	...
	while (esp8266_send_cmd("AT+CWJAP=\"MI\",\"12345678\"", "WIFI GOT IP", 500)){
		delay_ms(1);
		break;
	};
delay_ms(1000);
	...
	if (ack && waittime)	  //需要等待应答
	{
		while (--waittime) //等待倒计时
		{
			...
			if (USART2_RX_STA&0X8000) //接收到期待的应答结果
			{
				...
				USART2_RX_STA = 0;
			strcpy((char *)USART2_RX_BUF, "");		// 清空接收缓存区
			}
		}
		if (waittime == 0) res = 1;
	}
